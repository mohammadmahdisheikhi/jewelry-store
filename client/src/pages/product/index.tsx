				...
				const handleAddToBookmarks = async () => {
					...
					try {
						const response = await fetch(
							`http://localhost:8000/ad/add_bookmark/${id}/`,
							{
								method: "POST",
								headers: {
									Authorization: `Bearer ${token}`,
					...
					<>
						...
						<div className="md:flex mx-4 md:mx-8 lg:mx-20 my-10 md:space-x-4 gap-12">
							...
							<div className="md:w-1/2">
								...
								<span className="flex gap-2 font-light text-gray-600 my-4">
									نوع: <p className="font-medium text-black">{type}</p>
								</span>
								<span className="flex gap-2 font-light text-gray-600 my-4">
									وزن: <p className="font-medium text-black">{weight}</p>گرم
								</span>
								<span className="flex gap-2 font-light text-gray-600 my-4">
									عیار: <p className="font-medium text-black">{carat}</p>عیار
								</span>
								<span className="flex gap-2 font-light text-gray-600 my-4">
									مکان: <p className="font-medium text-black">{"تهران"}</p>
								</span>
								<span className="flex gap-2 font-light text-gray-600 my-4">
									قیمت: <p className="font-medium text-black">{price}</p>تومان
								</span>
								<span className="flex gap-2 font-light text-gray-600 my-4">
									توضیحات:{" "}
								...
								<div className="mt-8">
									<div className="flex  align-baseline">
										...
										<a
											href="/ads"
											className="font-extrabold text-sm text-secondary hover:text-gray-800 translate-y-[10px] mx-2"
										>
											بازگشت به آگهی‌ها
										</a>
									</div>
						...
						<div className="md:flex mx-4 md:mx-8 lg:mx-20 my-10 md:space-x-4 gap-12 justify-end">
							...
							<div className="p-4 md:w-1/2 space-y-4 border border-[1.5px] border-secondary rounded-sm">
								...
								<p className="flex">
									شماره تماس:<p className="font-bold mr-2">{sellerContact}</p>
								</p>
							</div>
						</div>
