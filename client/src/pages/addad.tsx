...
function Addad({ user }: { user: UserProfile | null }): JSX.Element {
  ...
  const initialValues: AdForm = { title: '', images: [], receipt_image: null, type: '', weight: '', carat: '', price: '' };
  ...
  const validationSchema: Yup.ObjectSchema<AdForm> = Yup.object({
    ...
    ).min(1, 'At least one image is required').required('Required'),
  ...
  const handleSubmit = async (values: AdForm, actions: any) => {
    ...
    try {
        ...
        if (values.receipt_image) {
            formData.append('receipt_image', values.receipt_image, values.receipt_image.name);
        }
        ...
        const response = await axios.post('http://localhost:8000/ad/create', formData, {
            headers: {
  ...
  const fields = [
    ...
    { name: 'images' as keyof AdForm, type: 'file', label: 'عکس طلا', multiple: true }, // Enable multiple files
    { name: 'receipt_image' as keyof AdForm, type: 'file', label: 'عکس کاغذ خرید' }, // Field for receipt image
