...
function Profile({ user }: { user: UserProfile | null }): JSX.Element {
	...
	const initialValues: LogisticFormValues = {
		address: "",
		shaba: "",
		postCode: "",
		city: "",
	...
	const fields = [
		{
			name: "city" as keyof LogisticFormValues,
			type: "text",
			label: "شهر شما",
		},
		{
			name: "address" as keyof LogisticFormValues,
			type: "text",
			label:
				"نشانی (نشانی شما برای تامین امنیت خریدها و ارسال آن‌ها ضروری است.)",
		},
		{
			name: "shaba" as keyof LogisticFormValues,
			type: "text",
			label: "شماره شبا (به منظور واریز مبالغ فروش و عودت وجه در صورت نیاز.)",
		},
		{
			name: "postCode" as keyof LogisticFormValues,
			type: "text",
			label: "کد پستی (به منطور تأیید نشانی)",
		},
	];
			...
			<div className="mx-4 md:mx-8 lg:mx-20 my-10">
				...
				<div className="md:grid grid-cols-2 md:gap-8 space-y-4 md:space-y-0">
					<div>
						...
						<span className="flex gap-2 text-gray-800">
							شماره ملی:
							<p className="text-black font-medium">{user.IDnumber}</p>
						</span>
						<span className="flex gap-2 text-gray-800">
							موجودی حساب کاربری شما:<p className="text-black font-medium"></p>
						</span>
					</div>
					<div>
...
export default function Personal(): JSX.Element {
	return <UserLoader>{(user) => <Profile user={user} />}</UserLoader>;
}
